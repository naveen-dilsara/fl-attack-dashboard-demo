...
def get_simulation_results():
    ...
    if sim_output is None:
        st.error("Simulation failed to run. Please check the console for error messages (e.g., 'Framingham.csv not found').")
        # Return a default dictionary with expected keys to prevent the dashboard from breaking completely
        return {
...
if st.sidebar.button("🔄 Re-run Simulation & Clear Cache"):
    st.cache_data.clear() # Clears the cache for get_simulation_results
    st.session_state.clear() # Clears session state to force re-run
...
if sim_results["initial_accuracy"] == 0.0 and not sim_results["feature_names"]: 
    st.stop() 
...
with tab3:
    ...
    if mse_data_dict: 
        ...
        st.warning("MSE values are not available (possibly due to simulation error or empty data).")
